...
async function fetchAllMatches(baseQuery) {
  const LIMIT = 500;
  const all = [];
  let offset = 0;
  while (true) {
    const q = `${baseQuery} limit ${LIMIT} offset ${offset}`;
    const data = await kintoneFetch('matches', `/k/v1/records.json?app=194&query=${encodeURIComponent(q)}`);
...
export async function handler(event, context) {
  ...
  try {
    if (event.httpMethod === 'GET') {
      ...
      if (qParams.seasonYear) {
        queryParts.push(`seasonYear = "${qParams.seasonYear}"`);
      }
      if (qParams.seasonQuarter) {
        queryParts.push(`seasonQuarter in ("${qParams.seasonQuarter}")`);
      }

      const baseQuery = queryParts.length > 0 ? queryParts.join(' and ') : '';
      const matches = await fetchAllMatches(baseQuery);
